...
|---|---|---|---|---|
| `visual-colored-left-border-strip` | MED | medi | regex | `border-l-4\s+border-(indigo\|purple\|blue\|teal\|violet)-[3-6]0…` |
...
| `ux-placeholder-as-primary-copy` | MED | low | regex | `placeholder=["'](Enter text( here)?\|Click here\|Type here\|Sel…` |
...
| `code-empty-catch-js` | HIGH | low | regex | `catch\s*\([^)]*\)\s*\{\s*\}` |
| `code-hardcoded-secret` | HIGH | medi | regex | `(api[_-]?key\|secret\|password\|token)\s*[:=]\s*['"][A-Za-z0-9_…` |
| `code-dead-branch-if-true` | MED | medi | regex | `if\s*\(\s*true\s*\)\|if\s+True\s*:` |
| `code-div-role-button` | MED | low | regex | `<div[^>]*\brole=["'](button\|checkbox\|link)["']` |
...
| `code-todo-stub-comments` | MED | medi | regex | `(//\|#\|/\*)\s*(TODO\|FIXME\|HACK\|XXX)\b` |
